...
async function backfillUserProfiles() {
  ...
  for (const user of usersWithoutProfile) {
    ...
    const company = `${prefix} ${name.replace(/[^a-zA-Z0-9 ]/g, '').split(' ').slice(0, 2).join(' ')}`;
    ...
    try {
      await prisma.userProfile.create({
        data: {
          userId: user.id,
          companyName: company,
          position: user.role === 'SUPPLIER' ? 'Direktur' : user.role === 'BUYER' ? 'Procurement Manager' : 'Staff',
          industry: user.role === 'SUPPLIER' ? 'Pertanian & Biomassa' : user.role === 'COURIER' ? 'Logistik' : 'Umum',
